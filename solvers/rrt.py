...
import constants
from solvers.KDTree    import KDTree
from solvers.utilities import gen_next, PathTree
...
class RRT(Solver):
    ...
    def _solve(self, env, timeout=None, info=None, vf=False):
        ...
        path = None
        clearance = constants.RRT_Collision_Clearance
        ...
        def policy(state):
            ...
            def extend_fn(pt1, pt2):
                ...
                extension = []
                for mid in np.arange(0, 1+constants.RRT_Extend_Granularity,
                                     step=constants.RRT_Extend_Granularity):
...
class BiRRT:
    ...
    """
    def __init__(self, sample_fn, extend_fn, collision_fn, distance_fn, seed):
    ...
    def query(self, pt1, pt2, timeout):
        ...
        for _ in range(constants.BiRRT_Num_Attempts):
    ...
    def update_collision_fn(self, collision_fn):
        self._collision_fn = collision_fn
    ...
    def _rrt_connect(self, pt1, pt2, start_time, timeout):
        ...
        for _ in range(constants.BiRRT_Num_Iters):
    ...
    def smooth_path(self, path):
        ...
        """
        for _ in range(constants.BiRRT_Smooth_Amt):
            if len(path) <= 2:
                return path
            i = self.rand_state.randint(0, len(path)-1)
            j = self.rand_state.randint(0, len(path)-1)
            if abs(i-j) <= 1:
                continue
            if j < i:
                i, j = j, i
...
class MRBiRRT:
    ...
    def query(self, pt1, pt2, step_size, timeout):
        """Query the BiRRT, to get a collision-free path from pt1 to pt2.
        """
        if self._collision_fn(pt1) is not None or \
           self._collision_fn(pt2) is not None:
        ...
        start_time = time.time()
        for _ in range(constants.BiRRT_Num_Attempts):
            path = self._rrt_connect(pt1, pt2, start_time, step_size, timeout)
            if path is not None:
                return path
            if timeout is not None and time.time()-start_time > timeout:
                break
    ...
    def _rrt_connect(self, pt1, pt2, start_time, step_size, timeout):
        KD = [KDTree(pt1), KDTree(pt2)]
        RRT = [PathTree(pt1), PathTree(pt2)]
        n = 1

        # Feature-related
        # fc = Features(plan_time, step_size)
        KD[n].initDistBetTrees(pt1, pt2)
        KD[n - 1].initDistBetTrees(pt1, pt2)
        ...
        while KD[0].length + KD[1].length < 1000:
            ...
            if last_expanded != n and q_new is not None:
                ...
                is_collision = False
                _q_near = q_near
                for newpt, flag in self._extend_fn(_q_near, q_next):
                    if self._collision_fn(newpt) is not None:
                        is_collision = True
                        break
                    if flag: _q_near = newpt

                if not is_collision:
                    # fc.add(total_trials, KD[n], q_near, q_next, 'towards other tree')  # Feature addition
                    KD[n].addNode(q_next)
                    KD[n].setDistBetTrees(q_next, KD[n - 1])
                    RRT[n].addPath(q_near, q_next)

                    if q_next == q_new: # Path found
                        path = []
                        ...
                        while cur.parent:
                            cur = cur.parent    # First cur is the same as the first cur in RRT[0] so we skip here
                            path.append(cur)
                        return [node.data for node in path]
            ...
            q_rand = self._sample_fn(pt1)   # Inside () has no effects so give anything

            q_near, dist = KD[n].nearestNode(q_rand)
            if dist < step_size:
                q_next = tuple(q_rand)
                # if trials < 10:   # Don't add q_rand if it is less than step_size away from q_next in the first this number of trials
                #     continue
            else:
                q_next = gen_next(q_near, q_rand, step_size)
                if self._collision_fn(q_next) is not None:  # failure due to sample collision
                    # fc.add(total_trials, KD[n], q_near, q_next, 'sample collision')  # Feature addition
                    continue
            ...
            for newpt, flag in self._extend_fn(_q_near, q_next):
                if self._collision_fn(newpt) is not None:
                    ...
                    break
                if flag: _q_near = newpt
            if is_collision:    # failure due to collision bet. q_near and q_next
                # fc.add(total_trials, KD[n], q_near, q_next, 'path collision')  # Feature addition
                continue

            # success of sampled point
            # fc.add(total_trials, KD[n], q_near, q_next, 'rand_success')  # Feature addition
            KD[n].addNode(q_next)
            KD[n].setDistBetTrees(q_next, KD[n - 1])
            RRT[n].addPath(q_near, q_next)
