...
class PickPlaceEnvironment(Environment):
    ...
    def _initial_pybullet_setup(self):
        ...
        p.setAdditionalSearchPath("utils/assets/")
        ...
        wall_d = 6.
        wall_h = 1.
        wall_t = constants.BINS_SHELF_BOX_T
        # for i in range(constants.BINS_SHELF_SLABS):
        # Left wall.
        link_cols.append(p.createCollisionShape(
            p.GEOM_BOX, halfExtents=[8, 0.01 / 2, wall_h / 2],
            physicsClientId=self._physics_client_id))
        link_vis.append(p.createVisualShape(
            p.GEOM_BOX, halfExtents=[8, 0.01 / 2, wall_h / 2],
            rgbaColor=(0.6, 0.3, 0.0, shelf_box_t),
            physicsClientId=self._physics_client_id))
        link_pos.append([0, 3, wall_h / 2])
        # Center wall.
        link_cols.append(p.createCollisionShape(
            p.GEOM_BOX, halfExtents=[8, 0.01 / 2, wall_h / 2],
            physicsClientId=self._physics_client_id))
        ...
        link_vis.append(p.createVisualShape(
            ...
            rgbaColor=(0.6, 0.3, 0.0, shelf_box_t),
            physicsClientId=self._physics_client_id))
        link_pos.append([0, -9, wall_h / 2])
        # Back wall.
        link_cols.append(p.createCollisionShape(
            p.GEOM_BOX, halfExtents=[0.01 / 2, 6, wall_h / 2],
            physicsClientId=self._physics_client_id))
        link_vis.append(p.createVisualShape(
            p.GEOM_BOX, halfExtents=[0.01 / 2, 6, wall_h / 2],
            rgbaColor=(0.6, 0.3, 0.0, shelf_box_t),
            physicsClientId=self._physics_client_id))
        link_pos.append([4, 0, wall_h / 2])
        p.createMultiBody(
            linkMasses=[10 for _ in link_pos],
            linkCollisionShapeIndices=link_cols,
            linkVisualShapeIndices=link_vis,
