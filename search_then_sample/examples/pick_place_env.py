...
from search_then_sample.utils.search_then_sample_utils import get_ik_ir_gen, base_motion, get_custom_limits, MergedPath, \
...
class PickPlaceEnvironment(Environment):
    ...
    def sample_IsValidPick(self, state, obj, rng=None):
        ...
        self.ik_ir_fn = get_ik_ir_gen(self.problem,
                                      custom_limits=get_custom_limits(self.robot, self.room_floors[0]))
    ...
    def sample_IsValidPlace(self, state, obj, rng=None):
        ...
        self.ik_ir_fn = get_ik_ir_gen(self.problem,
                                      custom_limits=get_custom_limits(self.robot, self.room_floors[1]))
    ...
    def sample_IsValidMove(self, state, obj, rng=None):
        ...
        custom_limits = get_custom_limits(self.robot, self.room_floors[1]) # TODO: currently hardcoded
        saved_world = WorldSaver()
        base_start = get_joint_positions(self.robot, joints_from_names(self.robot, PR2_GROUPS['base']))
